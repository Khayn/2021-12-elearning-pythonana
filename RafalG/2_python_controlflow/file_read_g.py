...
try:
    with open(FILE_GROUP) as file:
        etc_group = file.readlines()
except FileNotFoundError:
    print(f"File {FILE_GROUP} doesn't exist")
except PermissionError:
    print(f"Permission denied to file: {FILE_GROUP}")

try:
    with open(FILE_SHADOW) as file:
        etc_shadow = file.readlines()
except FileNotFoundError:
    print(f"File {FILE_SHADOW} doesn't exist")
except PermissionError:
    print(f"Permission denied to file: {FILE_SHADOW}")

try:
    with open(FILE_PASSWD) as file:
        etc_passwd = file.readlines()
except FileNotFoundError:
    print(f"File {FILE_PASSWD} doesn't exist")
except PermissionError:
    print(f"Permission denied to file: {FILE_PASSWD}")
...
for line in etc_group:
    ...
    # group_name, group_password, group_id, members = line.split(":")
    group_name, _, _, members = line.split(":")
    ...
    for member in members.split(","):
        if member not in result_group.keys():
            result_group[member] = list()  # member : []

        result_group[member].append(group_name)
        # created eg. {"lewis" : ["astronauts", "sysadmin", moon], etc}
...
for line in etc_shadow:
    ...
    timestamp = int(last_change) * DAY  # in seconds
